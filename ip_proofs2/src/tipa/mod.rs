...
pub struct TIPA2<IP, LMC, RMC, IPC, P, D> {
...
{
    dory_proof: DORYProof<IP, LMC, RMC, IPC, D>,
...
impl<IP, LMC, RMC, IPC, P, D> TIPA2<IP, LMC, RMC, IPC, P, D>
where
    D: Digest,
    P: PairingEngine,
    IP: InnerProduct<
    ...
    IPC::Output: MulAssign<P::Fr>,
{
    ...
    pub fn prove_with_srs_shift(
        ...
        // Run DORY
        let (proof, aux) = <DORY<IP, LMC, RMC, IPC, D>>::prove_with_aux(
            values,
    ...
    pub fn verify_with_srs_shift(
        ...
        let (base_com, transcript) =
            DORY::verify_recursive_challenge_transcript(com, &proof.dory_proof)?;
        let transcript_inverse = transcript.iter().map(|x| x.inverse().unwrap()).collect();
        ...
        let b_base = vec![proof.dory_proof.r_base.1.clone()];
...
mod tests {
    ...
    fn pairing_inner_product_test() {
        ...
        type PairingTIPA = TIPA2<IP, GC1, GC2, IPC, Bls12_381, Blake2b>;
    ...
    fn multiexponentiation_inner_product_test() {
        ...
        >;
        type MultiExpTIPA = TIPA2<IP, GC1, SC1, IPC, Bls12_381, Blake2b>;
    ...
    fn scalar_inner_product_test() {
        ...
        type ScalarTIPA = TIPA2<IP, SC2, SC1, IPC, Bls12_381, Blake2b>;
