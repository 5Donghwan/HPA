...
//TODO: Properly generalize the non-committed message approach of SIPP and MIPP to DORY
...
// Use placeholder commitment to commit to vector in clear during DORY execution
#[derive(Clone)]
...
pub struct DORYWithSSM<IP, LMC, IPC, D> {
...
impl<IP, LMC, IPC, D> DORYWithSSM<IP, LMC, IPC, D>
where
    D: Digest,
    ...
    IPC::Output: MulAssign<LMC::Scalar>,
{
    ...
    pub fn prove_with_structured_scalar_message(
        ...
        ck: (&[LMC::Key], &IPC::Key),
    ) -> Result<DORYProof<IP, LMC, SSMPlaceholderCommitment<LMC::Scalar>, IPC, D>, Error> {
        let (proof, _) =
            <DORY<IP, LMC, SSMPlaceholderCommitment<LMC::Scalar>, IPC, D>>::prove_with_aux(
                values,
                (
                    ck.0,
    ...
    pub fn verify_with_structured_scalar_message(
        ...
        scalar_b: &LMC::Scalar,
        proof: &DORYProof<IP, LMC, SSMPlaceholderCommitment<LMC::Scalar>, IPC, D>,
    ) -> Result<bool, Error> {
        // Calculate base commitments and recursive transcript
        //TODO: Scalar b not included in generating challenges
        let (base_com, transcript) = DORY::verify_recursive_challenge_transcript(
            (com.0, &LMC::Scalar::zero(), com.1),
            proof,
        )?;
        // Calculate base commitment keys
        let (ck_a_base, ck_b_base) = DORY::<IP, LMC, SSMPlaceholderCommitment<LMC::Scalar>, IPC, D>::_compute_final_commitment_keys(
            (&ck.0, &vec![HomomorphicPlaceholderValue {}; ck.0.len()], &ck.1),
            &transcript,
        )?;
        // Verify base commitment
        let dory_valid = DORY::_verify_base_commitment(
...
{
    dory_proof: DORYProof<IP, LMC, SSMPlaceholderCommitment<LMC::Scalar>, IPC, D>,
    final_ck: LMC::Key,
    ...
    LMC::Output: MulAssign<P::Fr>,
{
    fn clone(&self) -> Self {
        Self {
            dory_proof: self.dory_proof.clone(),
    ...
    LMC::Output: MulAssign<P::Fr>,
{
    ...
    pub fn prove_with_structured_scalar_message(
        ...
        // Run DORY
        let dory = start_timer!(|| "DORY");
        let (proof, aux) =
            <DORY<IP, LMC, SSMPlaceholderCommitment<P::Fr>, IPC, D>>::prove_with_aux(
                values,
                (
                    ck.0,
                    &vec![HomomorphicPlaceholderValue {}; values.1.len()],
    ...
    pub fn verify_with_structured_scalar_message(
        ...
        // Verify base inner product commitment
        let (com_a, _, com_t) = base_com;
        let a_base = vec![proof.dory_proof.r_base.0.clone()];
        let t_base = vec![IP::inner_product(&a_base, &vec![b_base])?];
...
mod tests {
    ...
    #[test]
    fn dory_ssm_scalar_inner_product_test() {
        ...
        type ScalarDORY = DORYWithSSM<IP, SC1, IPC, Blake2b>;

        let mut rng = StdRng::seed_from_u64(0u64);
        let (ck_a, ck_t) = ScalarDORY::setup(&mut rng, TEST_SIZE).unwrap();
        let mut m_a = Vec::new();
        ...
        let proof =
            ScalarDORY::prove_with_structured_scalar_message((&m_a, &m_b), (&ck_a, &ck_t)).unwrap();

        assert!(ScalarDORY::verify_with_structured_scalar_message(
            (&ck_a, &ck_t),
            (&com_a, &com_t),
            &b,
            &proof
