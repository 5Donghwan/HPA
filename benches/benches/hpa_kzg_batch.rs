...
fn bench_hpa_kzg<IP, CM, P, D, R: Rng>(rng: &mut R, len: usize)
where
    D: Digest,
    P: PairingEngine,
    IP: InnerProduct<
    ...
    // LMC::Output: MulAssign<LMC::Scalar>,
{
    ...
    let (u1, u2) = HPA::<IP, CM, P, D>::set_values(&v1, &v2).unwrap();
    ...
    let mut bat_proof =
        HPA::<IP, CM, P, D>::prove(&srs, (&(bat_v1.clone()), &(bat_v2.clone()), &(bat_w_vec.clone())),
        //  &hpa_srs, 
